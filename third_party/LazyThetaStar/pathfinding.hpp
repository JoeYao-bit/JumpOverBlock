...
//            return f < rhs.f;
        ...
        std::vector<DataType>
        search(const DataType &start, const DataType &end, std::function<DataType(const NodeId)> idToData,
    ...
    private:
        ...
        void addToOpen(NodeId id) {
            ...
            if (nodes[id].list == OPEN_LIST) {
                auto index = std::find_if(openList.begin(), openList.end(),
                                          [&](const auto &heap) { return heap.id == id; });
                auto id = index->id;
