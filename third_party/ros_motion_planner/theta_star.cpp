...
{
  freeNav::Pointi<2> pt1({child.x_, child.y_}), pt2({parent.x_, parent.y_});
...
freeNav::Path<2> ThetaStarRimJump(const unsigned char* global_costmap, freeNav::DimensionLength* dim,
    ...
    si.id_ = theta_star.grid2Index(si.x_, si.y_);
    ti.x_ = target[0], ti.y_ = target[1];
    ti.id_ = theta_star.grid2Index(ti.x_, ti.y_);
    std::vector<Node> path, nodes;
    if(theta_star.plan(global_costmap, si, ti, path, nodes)) {
