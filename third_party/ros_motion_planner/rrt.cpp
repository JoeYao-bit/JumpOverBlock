...
//  // obstacle check, yz move it before
//  if (_isAnyObstacleInPath(new_node, nearest_node))
//    new_node.id_ = -1;
//
  return new_node;
...
{
    freeNav::Pointi<2> pt1({n1.x_, n1.y_}), pt2({n2.x_, n2.y_});
    return !line_collision_check_(pt1, pt2);
...
bool RRT::_checkGoal(const Node& new_node)
{
  ...
  return false;
    ...
    freeNav::Path<2> RRTRimJump(const unsigned char* global_costmap, freeNav::DimensionLength* dim, double resolution,
        ...
        Node si, ti;
        si.x_ = start[0], si.y_ = start[1];
        si.id_ = planner.grid2Index(si.x_, si.y_);
        ti.x_ = target[0], ti.y_ = target[1];
        ti.id_ = planner.grid2Index(ti.x_, ti.y_);
        std::vector<Node> path, nodes;
        if(planner.plan(global_costmap, si, ti, path, nodes)) {
