...
namespace freeNav::JOB {
    ...
    // update inner index of line
    template<Dimension N>
    float findExitPointOfBlockFloat(Line<float, N>& line,
                                    const Pointf<N>& current_pt,
                                    const int& index,
                                    const Pointi<N>& min_pt,
                                    const Pointi<N>& max_pt) {
        // check whether the line reach end of line
        if(index >= line.step - 1) { return 0; }
        //Pointi<N> current_pt = line.GetPoint(index);
        // check whether current line's last traveled point in the block
        ...
        //bool line_increase = (line.step_length > 0);
        //Dimension minimum_step_exit_dim = 0;
        float minimum_step_to_exit = line.step, future_step;
        // determine the fast dim to leave current block
        for(Dimension dim=0; dim<N; dim++) {
            if(line.parameter[dim].second == 0) {
                continue;
                ...
                if(line.parameter[dim].second > 0) {
                    future_step = fabs((max_pt[dim] - current_pt[dim]) / line.parameter[dim].second);
                } else {
                    future_step = fabs((min_pt[dim] - current_pt[dim]) / line.parameter[dim].second);
                }
                //std::cout << " dim " << dim << " / future_step " << future_step.toFloat() << std::endl;
                if (future_step < minimum_step_to_exit) {
                    //minimum_step_exit_dim = dim;
    ...
    // update inner index of line
    template<Dimension N>
    float findExitPointOfBlockFloat(Line<float, N>& line, const Pointf<N>& current_pt, const int& index, const BlockPtr<N>& block_ptr) {
        return findExitPointOfBlockFloat(line, current_pt, index, block_ptr->min_, block_ptr->max_);
    }

    template <Dimension N>
    bool LineCrossObstacleWithBlockJumpFloat(const Pointf<N>& pt1, const Pointf<N>& pt2,
                                        BlockDetectorInterfacePtr<N> block_detector_ptr,
                                        std::vector<Pointi<N> >& visited_pt,
                                        int& count_of_block) {
        if(pt1 == pt2) { return true; }
        visited_pt.clear();
        count_of_block = 0;
        Line<float, N> line(pt1, pt2);
        int check_step = line.step;
        Pointf<N> pt; Pointi<2> pti;
        Id current_id;
        int jump_step = 0;
        for(int i=1; i<check_step; i++) {
            pt = line.GetPointFloat(i);
            pti = pt.toInt();
            visited_pt.push_back(pti);
            if(block_detector_ptr->is_occupied_(pti)) {
                return true;
            }
            //if(is_occupied(pt)) { return true; }
            current_id = PointiToId(pti, block_detector_ptr->dimension_info_);
            const auto& block_ptr = block_detector_ptr->block_ptr_map_[current_id];
            // if in block, jump over current block
            if(block_ptr != nullptr) {
                jump_step = findExitPointOfBlockFloat(line, pt, i, block_ptr);
                //std::cout << " jump step " << jump_step << std::endl;
                i = i + jump_step;
                count_of_block ++;
            }
