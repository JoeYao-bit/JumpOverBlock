...
//Signal: SIGABRT (Aborted)
                            ...
                            std::vector<Pointf<2> >& visited_fpt)
    {
        ...
        float b_min_x,b_min_y;
        float b_max_x,b_max_y;
        float t_x_low,t_y_low;
        float t_x_high,t_y_high;
        ...
        while (x >= 0 && x < gridW && y >= 0 && y < gridH)
        {
            ...
            if (!step_by_step && current_block != nullptr) {
                ...
                if(std::fabs(dir[0]) > eps)
                {
                    b_min_x = static_cast<float>(block_min[0]);
                    b_max_x = static_cast<float>(block_max[0]) + 1.0f;
                    t_x_low  = (b_min_x - start[0]) / dir[0];
                    t_x_high = (b_max_x - start[0]) / dir[0];
                    if(dir[0] > 0)
                ...
                if(std::fabs(dir[1]) > eps)
                {
                    b_min_y = static_cast<float>(block_min[1]);
                    b_max_y = static_cast<float>(block_max[1]) + 1.0f;
                    t_y_low  = (b_min_y - start[1]) / dir[1];
                    t_y_high = (b_max_y - start[1]) / dir[1];
                    if(dir[1] > 0)
    ...
    bool lineOfSightCheckAW(const Point<T,2>& start, const Point<T, 2>& end,
        ...
        // 2. 纯垂直线：x全程不变，只遍历y轴
        BlockPtr<2> current_block = nullptr;
        if (std::fabs(dir[0]) < eps) {
            //std::cout << "case 1 " << std::endl;
            int fixedX = static_cast<int>(std::floor(start[0]));
            float yMinF = std::min(start[1], end[1]);
            ...
            for (int y = yStart; y <= yEnd; ++y) {
                ...
                if (fixedX >= 0 && fixedX < gridW && y >= 0 && y < gridH)
                {
                    ...
                    current_id = PointiToId(current_pt, block_detector_ptr->dimension_info_);
                    current_block = block_detector_ptr->block_ptr_map_[current_id];
                    if(current_block != nullptr) {
                        if (current_block->max_[1]+1 > yEnd) {
                            return false;
                        } else {
        ...
        while (x >= 0 && x < gridW && y >= 0 && y < gridH) {
            ...
            if (!step_by_step && current_block != nullptr) {
                ...
                t_block_exit = std::min(t_candidate_x, t_candidate_y);
